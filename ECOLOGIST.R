...
#Read EMA Data:
emadata.raw<- read.csv(file.choose(), stringsAsFactors=FALSE) #find function
...
emaplot.percent<-ggplot(data = emamelt.merge, aes(x=date, y=porp, group=Type, shape=Type, color=Type)) +
  ...
  geom_point()+
  geom_label_repel(data = emamelt.merge[(which(emamelt.merge$date %in% c(startdate+7,startdate+14,startdate+21,ifelse(enddate>Sys.Date(),Sys.Date(),NULL)))),], aes(x=date, y=porp,label=percentage))

ggsave(paste(Initial,"_EMAPro_PercentPlot.jpeg",sep = ""),device = "jpeg",plot = emaplot.percent,dpi = 300,path = NULL)
...
emaplot.count<-ggplot(data = emamelt.merge, aes(x=date, y=actual, color=Type, group=Type, shape=Type)) +
  ...
  scale_x_date(name="Date",limits = c(startdate+1,NA) ,date_breaks = "2 days")+
  geom_point()+
  geom_label_repel(data = emamelt.merge[(which(emamelt.merge$date %in% c(startdate+7,startdate+14,startdate+21,ifelse(enddate>Sys.Date(),Sys.Date(),NULL)))),], aes(x=date, y=actual,label=actual))+
  geom_label_repel(data = emamelt.merge[(which(emamelt.merge$date %in% c(startdate+7,startdate+14,startdate+21,ifelse(enddate>Sys.Date(),Sys.Date(),NULL)) & emamelt.merge$Type==c("BoD","DoD"))),], aes(x=date, y=expectation,label=expectation))

ggsave(paste(Initial,"_EMAPro_CountPlot.jpeg",sep = ""),device = "jpeg",plot = emaplot.count,dpi = 300,path = NULL)
