...
for (form_i in 8) {
  ...
  if (length(fm_dir)>1){
    #STEP1.2 Grab forms, remove col 'X', combine forms by common cols and check if common cols have same values. If not need to combine multiple forms, jump to STEP1.3. 
    ...
    if (length(temp_dup_id)>0){
      ...
      if(as.logical(remove_dupid)){
        ...
        if(length(temp_var_date)>1){stop(message('Is the second col the same (should be the date)?'))}
    ...
    rm(comb_fm_list,temp_comm_col_list,temp_dup_id,fm_dir,temp_chck_dupid,temp_confirm,temp_var_date,remove_dupid,temp_confirm,temp_na_in_comm_col) #keep comm_var, form_i, formname, vm, acvar_or, acvar_chk
