# remove unnecessary rows so that ID+CDATE are unique 
setwd("~/Documents/github/UPMC/TRANSFER/exit wtar mdrs/")

proc_neuropsy_s <- function(dfx,stoppoint = 3, DataStartPoint = 6){
  vari_index <- list(
    cdate_varis = names(dfx)[1:stoppoint],
    de_varis = names(dfx)[(DataStartPoint):length(dfx)]
  )
...
# read and pre-process data, run proc_neuropsy_s function, get cleaned form, get cdate (use DE as cdate if DE>CDATE+100)
...
for (form_i in 1:length(forms)) {
  ...
  STEP1(RAWDATA = list(cleaned_exit,cleaned_wtar)[[form_i]]) # get 'raw_nonch': redcap variables, 
  if(!skipotherforms|ifbl){ # skip the form if skipotherforms is T AND ifbl is F
    STEP2();STEP3()
