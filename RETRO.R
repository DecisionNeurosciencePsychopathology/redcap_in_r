...
dnpl.eprimeread<-function(fpath,getvar=NULL,filter.x=T,...){
  if (missing(fpath)){
    fpath<-file.choose()}
...
if(anyNA(match(getvar.tu,dfname.tu))) {readline(prompt = "Not a type of data in this file, redefine get var")}
...
tdf.finale$Eprime.FrameNumber<-as.numeric(tdf.finale$Eprime.FrameNumber)
tdf.finale<-tdf.finale[order(as.numeric(tdf.finale$Eprime.FrameNumber)),]
rownames(tdf.finale)<-NULL
